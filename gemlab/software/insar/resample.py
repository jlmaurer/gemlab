...
def resample_delays(
    ...
    *,
    ...
    path_groups: list[list[Path]] = [
        ...
        for path_group in (
            unw_paths,
            list(delays_dir.glob('**/*hydro*.tif')),
            list(delays_dir.glob('**/*wet*.tif')),
        )
        if len(path_group) > 0
    ]
    resample(
...
def ifg_matches(needle_parts: list[str], candidate_parts: list[str]) -> bool:
    """Format:
    S1AA_20200920T001203_20201002T001203_***
    """
    return (
...
def get_start_date_str(parts: list[str]) -> str:
    if parts[0] == 'S1AA':
        return parts[1].split('T')[0]
    return parts[2].split('T')[0]


def delay_matches(needle_parts: list[str], candidate_parts: list[str]) -> bool:
    """Format:
    S1AA_20200920T001203_20201002T001203_*
    or
    <WM name>_<hydro/wet>_20200920T230000_*
    """
    if needle_parts[0] == candidate_parts[0] == 'S1AA':
        return ifg_matches(needle_parts, candidate_parts)
    needle_start_date = get_start_date_str(needle_parts)
    candidate_start_date = get_start_date_str(candidate_parts)
    return needle_start_date == candidate_start_date
...
def find_matching_file(haystack: Iterable[Path], needle: Path) -> Path | None:
    ...
    for candidate in haystack:
        ...
        if delay_matches(needle_parts, candidate_parts):
            return candidate
    return None
...
if __name__ == '__main__':
    ...
    args = p.parse_args()
    resample_interferograms(**args.__dict__)
