...
def to_timestamp(date: dt.date) -> float:
    """Seconds since the Unix Epoch for a dt.date."""
    return time.mktime(date.timetuple())


def from_timestamp(timestamp: float) -> dt.datetime:
    return dt.datetime.fromtimestamp(0) + dt.timedelta(seconds=timestamp)
...
def get_year_frac(date: dt.date) -> float:
    ...
    seconds_elapsed = to_timestamp(date) - to_timestamp(start_of_this_year)
    seconds_in_this_year = (
        to_timestamp(start_of_next_year) - to_timestamp(start_of_this_year)
    )
...
def dereference(
    ...
    *,
    ...
    for k in range(len(ifgs)):
        ifgs[k] -= np.nanmean(
            ifgs[k, list(range(w, e)), list(range(n, s))]
        )
