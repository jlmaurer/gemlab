...
def cmdLineParse():
    ...
    parser.add_argument('-sn', '--stepnumber', type=int, metavar='', required=False, help='1 for step1, 2 for step2, 3 for step 3, 4 for step4')
    parser.add_argument('-rlks', '--rangelooks', type=int, metavar='', required=True, help='multilook factor for SAR range direction')
    parser.add_argument('-alks', '--azimuthlooks', type=int, metavar='', required=True, help='multilook factor for SAR azimuth direction')
...
def step1(fringefolder,stackfolder,S,N,W,E,stacksize):
    ...
    linetowrite6 = "tops2vrt.py -i ../merged/ -s coreg_stack -g geometry -c slcs -B " + S + " "+ N + " "+W+ " "+ E+"\n\n"
    linetowrite7 = "nmap.py -i coreg_stack/slcs_base.vrt -o KS2/nmap -c KS2/count -x 11 -y 5\n\n"
    linetowrite8 = "sequential.py -i ../merged/SLC -s "+np.str(stacksize) +" -o Sequential -w KS2/nmap -b coreg_stack/slcs_base.vrt -x 11 -y 5\n\n"
...
def step2(fringefolder,stacksize):
    ...
    linetowrite3 = "#SBATCH --mail-user=yl3mz@mst.edu\n#SBATCH --export=all\n#SBATCH --out=Foundry-%j.out\n"
    linetowrite4 = "#SBATCH --mem-per-cpu=4000\n#SBATCH -p general\n\n"
    linetowrite5 = "adjustMiniStacks.py -s slcs/ -m Sequential/miniStacks/ -d Sequential/Datum_connection/ -M "+ np.str(stacksize)+" -o adjusted_wrapped_DS\n\n"
    linetowrite6 = "ampdispersion.py -i coreg_stack/slcs_base.vrt -o ampDispersion/ampdispersion -m ampDispersion/mean\n\n"
    ...
    linetowrite10 = "unwrapStack.py -s slcs -m Sequential/miniStacks/ -d Sequential/Datum_connection/ -M "+np.str(stacksize)+" -u 'unwrap_fringe.py' --unw_method snaphu\n\n"
...
def step3(fringefolder, stackfolder,rlks,alks):
    '''
    ...
    linetowrite2 = "#SBATCH --time=5-00:00:00\n#SBATCH --mail-type=begin,end,fail,requeue\n"
    linetowrite3 = "#SBATCH --mail-user=yl3mz@mst.edu\n#SBATCH --export=all\n#SBATCH --out=Foundry-%j.out\n"
    linetowrite4 = "#SBATCH --mem-per-cpu=80000\n##SBATCH -p general\n\n"
    #linetowrite5 = 'rm *rlks' + str(rlks) + '*alks' + str(alks) + '*int\n'
    linetowrite6 = 'ls *int >  list_int.txt\n\n'
    awk1 = "awk '{print " 
    awk2 = '"multilook.py "substr($1,1,17)".int -r '+str(rlks)+' -a '+str(alks)+' -o "substr($1,1,17)"_rlks'+str(rlks)+'_alks'+str(alks)+'.int"}' 
    ...
    linetowrite11 = "multilook.py  tcorr_ds_ps.bin -r "+str(rlks)+" -a "+str(alks)+" -o tcorr_ds_ps_rlks"+str(rlks)+"_alks"+str(alks)+".bin\n\n"
    linetowrite12 = "ls *rlks"+str(rlks)+"*alks"+str(alks)+"*int > multilook_int.txt\n\n"
    awk1 = "awk '{print "
    awk2 = '"unwrap_fringe.py -m snaphu -i "$1" -c tcorr_ds_ps_rlks'+str(rlks)+'_alks'+str(alks)+'.bin -o unwrap/"substr($1,1,17)".unw"}'
    ...
    awk1 = 'export WDIR='
    awk2 = stackfolder + '/' + PSDSfolder +'\n'
    linetowrite15 = awk1 + awk2

    awk1 = 'scontrol show hostname $SLURM_JOB_NODELIST |'
    awk2 = " perl -ne 'chomb; print " + '"$_"x1' + " ' > hostfile \n"
    linetowrite16 = awk1 + awk2 

    linetowrite17 = 'echo "finished job"'
    ...
    bsub_file.writelines(linetowrite4)
    #bsub_file.writelines(linetowrite5)
    bsub_file.writelines('export PATH=/home/yl3mz/anaconda3/envs/isce2/bin:$PATH\n')
    bsub_file.writelines('module load anaconda\n')
    bsub_file.writelines('source activate  /home/yl3mz/anaconda3/envs/isce2\n\n')
    bsub_file.writelines(linetowrite6)
    bsub_file.writelines(linetowrite7)
    bsub_file.writelines(linetowrite8)
    bsub_file.writelines(linetowrite9)
    ...
    bsub_file.writelines(linetowrite15)
    bsub_file.writelines('echo $WDIR\n\n')
    bsub_file.writelines('. `which env_parallel.bash`\n')
    bsub_file.writelines(linetowrite16)
    bsub_file.writelines('env_parallel -j $SLURM_CPUS_ON_NODE --slf hostfile --wd $WDIR < run_unwrap.txt\n\n')
    bsub_file.writelines(linetowrite17)
    bsub_file.close()   
...
def step4(fringefolder, stackfolder, rlks, alks,S,N,W,E):
    ...
    linetowrite11 = "multilook.py hgt.rdr.full -r "+str(rlks)+" -a "+str(alks)+" -o hgt_rlks"+str(rlks)+"_alks"+str(alks)+".rdr\n"
    linetowrite12 = "multilook.py lon.rdr.full -r "+str(rlks)+" -a "+str(alks)+" -o lon_rlks"+str(rlks)+"_alks"+str(alks)+".rdr\n"
    linetowrite13 = "multilook.py lat.rdr.full -r "+str(rlks)+" -a "+str(alks)+" -o lat_rlks"+str(rlks)+"_alks"+str(alks)+".rdr\n"
    linetowrite14 = "multilook.py los.rdr.full -r "+str(rlks)+" -a "+str(alks)+" -o los_rlks"+str(rlks)+"_alks"+str(alks)+".rdr\n"
    linetowrite15 = "multilook.py incLocal.rdr.full -r "+str(rlks)+" -a "+str(alks)+" -o incLocal_rlks"+str(rlks)+"_alks"+str(alks)+".rdr\n"
    linetowrite16 = "multilook.py shadowMask.rdr.full -r "+str(rlks)+" -a "+str(alks)+" -o shadowMask_rlks"+str(rlks)+"_alks"+str(alks)+".rdr\n\n"
    ...
    for ind, val in enumerate(layers):
        with open( os.path.join(outfilename, val+'.vrt'), 'w') as fid:
            fid.write( vrttmpl2.format( xsize = xsize, ysize = ysize,
                                       xmin = xmin, ymin = ymin,
                                       width = width,
                                       height = height,
                                       PATH = os.path.abspath( os.path.join(stackfolder,'merged', 'geom_reference', val+fname3))))
